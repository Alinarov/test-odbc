...
<head>
    ...
    <title>Documentación del Código en D</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>Documentación del Código en D</h1>
        <pre><code>
#!/usr/bin/env dmd
module comunicacion;
import std;
...
class comunicaicon {
    ...
    string[][] ejecutar_query(string comando) {
        ...
        if (SQL_SUCCEEDED(ret = SQLDriverConnect(conn, null, cast(char*) toStringz(connectionString), SQL_NTS, null, 0, null, SQL_DRIVER_NOPROMPT))) {
            ...
            // Aquí recibimos el comando SQL o query SQL para poder mandarlo al SQL Server para que lo ejecute
            ...
            // Aquí solo desconectamos el handle o sea el cursor que equivale en cursor.close() en Python
    ...
    void cerrar_conexion() {
        // Esta función no usar a no ser que sea necesario, cierra la conexión con el servidor
        SQLDisconnect(conn);
